...
let main_interactive() = 
  let c1 = new connection ~conninfo () in
  print_conn_info c1; flush stdout;
  c1#set_notice_processor (fun s -> eprintf "postgresql error [%s]\n" s); 
  try
...
let main () =
 ...
 (* let chan= if (Array.length Sys.argv) = 1 then stdin else open_in Sys.argv.(2) in *)
   if !interactive then (main_interactive()) else (
